...
export class UpdateConcernPage {
  static get parameters() {
    return [[ViewController], [NavParams], [NavController], [UpdateService]];
  }

  constructor(viewCtrl, navParams, nav, updateService) {
    this.viewCtrl = viewCtrl;
    this.navParams = navParams;
    this.nav = nav;
    this.navigatedFromPage = navParams.get('page');
    // load the JSON string of the ID Pair selected by the user
    this.requestedIDPair = JSON.parse(navParams.get('selectedIDPair'));
    ...
    this.updateTipID = this.requestedIDPair.tip_id;
    ...
    if (this.updateLoadComplete) {
        ...
        if (this.updateInput.length > 0) {
            ...
            let alert = Alert.create({
                title: "Thank you!",
                message: "Thank you for checking on your existing Concern and helping build a better workplace neighborhood!",
                buttons: [{
                    text: 'Okay',
                    handler: () => {
  ...
  navToConcernReceived() {
    this.nav.push(ConcernReceivedPage, {
      page: this,
      tip_id: this.requestedIDPair.tip_id
    });
  }
}
